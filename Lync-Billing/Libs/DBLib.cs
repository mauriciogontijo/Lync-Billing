    ...
    public class DBLib
    {
        ...
        public DataTable SELECT(string tableName,List <string> fields, Dictionary<string, object> whereClause, int limits)
        {
           ...
           if(whereClause.Count != 0)
           {
            ...
            {
                if (whereClause !=null)
                    selectQuery = string.Format("SELECT {0} FROM  [{1}] {2} COLLATE SQL_Latin1_General_CP1_CI_AS {3}", selectedfields.ToString(), tableName, whereStatement.ToString(), orderBy);
                else
                    selectQuery = string.Format("SELECT {0} FROM  ['{1}'] COLLATE SQL_Latin1_General_CP1_CI_AS {2}", selectedfields.ToString(), tableName, orderBy);
            }
            else
            {
                if (whereClause != null)
                    selectQuery = string.Format("SELECT TOP({0}) {1} FROM [{2}] {3} COLLATE SQL_Latin1_General_CP1_CI_AS {4}", limits, selectedfields.ToString(), tableName, whereStatement.ToString(), orderBy);
                else
                    selectQuery = string.Format("SELECT TOP({0}) {1} FROM [{2}] COLLATE SQL_Latin1_General_CP1_CI_AS {3}", limits, selectedfields.ToString(), tableName, orderBy);
            }
