        ...
        public static List<UsersCallsSummary> GetUsersCallsSummary1(string sipAccount, int year, int month) 
        {
            ...
            foreach (DataRow row in dt.Rows) 
            {
                ...
                if (row[dt.Columns["ui_IsPersonal"]] != System.DBNull.Value && row[dt.Columns["ui_IsPersonal"]].ToString() == "NO")
                {
                    ...
                    if (row[dt.Columns["MonthlyDuration"]] != System.DBNull.Value)
                        userSummary.BusinessCallsDuration = Convert.ToInt32(row[dt.Columns["MonthlyDuration"]]);
                    else
                        userSummary.BusinessCallsDuration = 0;
                }
                else if (row[dt.Columns["ui_IsPersonal"]] != System.DBNull.Value && row[dt.Columns["ui_IsPersonal"]].ToString() == "YES")
                {
                    ...
                    if (row[dt.Columns["MonthlyDuration"]] != System.DBNull.Value)
                        userSummary.PersonalCallsDuration = Convert.ToInt32(row[dt.Columns["MonthlyDuration"]]);
                    else
                        userSummary.PersonalCallsDuration = 0;
                }
